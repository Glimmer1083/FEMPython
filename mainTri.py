...
@LastEditTime: 2020-07-04 17:12:28
...
from triElement import *

nodeFile = "./mesh3/E.n"
sideFile = "./mesh3/E.s"
elementFile = "./mesh3/E.e"
...
def read_node(filename):
    with open(filename, 'r') as file_to_read:
        ...
        for index in range(nodeNum):
            ...
            item = [i for i in lines.split()]
            node = Node(
                int(item[0]), float(item[1]), float(item[2]), int(item[3]))
            nodes.append(node)
    return nodes
...
def read_side(filename, nodes):
    with open(filename, 'r') as file_to_read:
        ...
        for index in range(sideNum):
            ...
            item = [i for i in lines.split()]
            side = Side(
                int(item[0]), [nodes[int(item[1])], nodes[int(item[2])]],
                int(item[5]))
            sides.append(side)
    return sides
...
def create_elements(filename, nodes, sides):
    with open(filename, 'r') as file_to_read:
        ...
        for index in range(elementNum):
            ...
            element = Element(
                int(item[0]), [
                    nodes[int(item[1])], nodes[int(item[2])], nodes[int(
                        item[3])]
                ], [
                    sides[int(item[7])], sides[int(item[8])], sides[int(
                        item[9])]
                ])
    ...
    for i in range(3):
        for j in range(3):
            if Ae_[element.node[i].number, element.node[j].number] == 0:
                Ae_[element.node[i].number, element.node[j].
                    number] = element.Ae[i, j]
            else:
                Ae_[element.node[i].number, element.node[j].
                    number] += element.Ae[i, j]
    for i in range(3):
        if fe_[element.node[i].number, 0] == 0:
            fe_[element.node[i].number, 0] = element.elementLoadVector[
                i, 0] + element.lineLoadVector[i, 0]
        else:
            fe_[element.node[i].number, 0] += (
                element.elementLoadVector[i, 0] + element.lineLoadVector[i, 0])
    # pass
